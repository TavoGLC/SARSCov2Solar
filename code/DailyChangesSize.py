...
data = pd.read_csv('/media/tavo/storage/biologicalSequences/covid/datasets/MetaDataNASA.csv')
data = data[data['correctdata']==1]
data = data.set_index('id')

data = data[data['qry']!='lat==0.0 & long==0.0']
...
r0 = data.groupby('date')['Length'].mean().rolling(14)
r1 = data.groupby(pd.qcut(data['lengthofday'],365,duplicates='drop'))['Length'].mean().rolling(14)
r2 = data.groupby('spots')['Length'].mean().rolling(14)

###############################################################################
# Loading packages 
###############################################################################

cities = data['qry'].value_counts().index

city_loc = cities[0]
sdata = data[data['qry']==city_loc].copy()
...
for k,val in enumerate(days):
    ...
    a,b = np.histogram(miniData['Length'],bins=100)
    b = [(b[j]+b[j+1])/2 for j in range(len(b)-1)]
    a = (a - a.min())/(a.max() - a.min())
    ax.plot(b,4*a+k,color='black',alpha=0.25)

ax.set_xlim(29500,29920)
ax.set_ylim(0,370)
ax.set_xlabel("Mean Genome of Size",fontsize=fontsize)
...
ldays = sdata['lengthofday'].unique()

for k,val in enumerate(ldays):
    miniData = sdata[sdata['lengthofday']==val]
    a,b = np.histogram(miniData['Length'],bins=100)
    b = [(b[j]+b[j+1])/2 for j in range(len(b)-1)]
    a = (a - a.min())/(a.max() - a.min())
...
data.groupby('dayofyear')['Length'].mean().diff().plot(ax=ax,color='navy')
...
rolling_a = data.groupby('dayofyear')['Length'].mean().diff().rolling(daily_rolling).std()
...
PlotStyle(ax)
plt.tight_layout()
plt.savefig('/media/tavo/storage/biologicalSequences/covidsr2/images/image_daily.png',dpi=75,bbox_inches='tight')
plt.close()
