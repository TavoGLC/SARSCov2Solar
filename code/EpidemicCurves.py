...
for k in range(10):
    ...
    axs[k].set_xlabel('Normalized \n Time Scale',fontsize=14)
    axs[k].legend(loc=1)
    PlotStyle(axs[k])

axs[0].text(0.01, 0.99, 'A', size=25, color='black', ha='left', va='top', transform=axs[0].transAxes)

axmp = fig.add_subplot(gs[1:8,0:7])

m = Basemap(projection='cyl',llcrnrlat=-80, urcrnrlat=80,fix_aspect=False,
...
sctr = m.scatter(gr_locationdata['long'].values,gr_locationdata['lat'].values,c=coefs,
          s=gr_locationdata['cases'].values/5,cmap='viridis')
axmp.set_frame_on(False)

cbaxes = axmp.inset_axes([0.015,0.1,0.25,0.025])

cbar = plt.colorbar(sctr,cax=cbaxes,orientation='horizontal')
...
for i,(lt, dfr) in enumerate(zip(selectedlats,dataframes)):
    ...
    axlats.plot(np.array(localdf.index),(lt+15*np.array(localdf)),alpha=0.025,color='gray')
...
plt.savefig('/media/tavo/storage/biologicalSequences/covidsr2/images/image_map.png',dpi=75,bbox_inches='tight')