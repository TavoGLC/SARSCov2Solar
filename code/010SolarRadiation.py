...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 14 16:51:56 2023
...
with open('/media/tavo/storage/biologicalSequences/covidsr04/data/solar/tsis_ssi_L3_c24h_v10_20180314_20230722.txt') as f:
    lines = f.readlines()
...
wldata['date'] = [val[0:-3] for val in datasplited[:,0]]
wldata['date'] = pd.to_datetime(wldata['date'])

wldata['wavelength'] = datasplited[:,2].astype(np.float32)
wldata['instrument_mode_id'] = datasplited[:,3].astype(np.int16)
wldata['data_version'] = datasplited[:,4].astype(np.int16)
wldata['irradiance'] = datasplited[:,5].astype(np.float64)
wldata['instrument_uncertainty'] = datasplited[:,6].astype(np.float64)
wldata['measurement_precision'] = datasplited[:,7].astype(np.float64)
wldata['measurement_stability'] = datasplited[:,8].astype(np.float64)
wldata['quality'] = datasplited[:,9].astype(np.float64)
...
outdata.to_csv('/media/tavo/storage/biologicalSequences/covidsr04/data/solar/solarcurrent.csv')
