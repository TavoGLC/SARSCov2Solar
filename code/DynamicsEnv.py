...
def MakeShapePlot(Rolling,labelA,labelB,shapeColumns,ax,scale=0.05):
    ...
    if type(colordata[0])!=np.float64:
...
yearSolar = data.groupby(pd.qcut(data['lengthofday'],365,duplicates='drop')).mean(numeric_only=True).rolling(15)
solar = data.groupby('spots').mean(numeric_only=True).rolling(15)
...
for k,val in enumerate([yearCalendar,yearSolar,solar]):
    ...
    if type(val.index[0])!=np.float64:
...
plt.tight_layout()
plt.savefig('/media/tavo/storage/biologicalSequences/covidsr2/images/image_dynenv.png',dpi=75,bbox_inches='tight')
plt.close()