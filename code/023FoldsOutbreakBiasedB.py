...
reSamplingIndex = GetEqualizedIndex(MetaData['outbreakdays'],maxCount=180,bins=1276)
miniData = MetaData.iloc[reSamplingIndex]

trainInx = miniData.index.values.copy()
np.random.seed(46845)
...
outdir = '/media/tavo/storage/biologicalSequences/covidsr04/data/folds/BiasedB'
...
for k,fld in enumerate(folds.split(trainInx)):
    ...
    trainDF['Fold'+str(k)] = trainInx[train_index][0:154590]
    testDF['Fold'+str(k)] = trainInx[test_index][0:38640]
