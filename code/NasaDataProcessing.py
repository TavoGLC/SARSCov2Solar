...
outfiledir = '/media/tavo/storage/nasa_npy/'
basedir = '/media/tavo/storage/nasa_data'
files = np.sort(os.listdir(basedir))

img_dirs = ['/media/tavo/storage/dtaimg/0'+str(k+1) for k in range(14)]

files = np.sort(os.listdir(basedir))

filePaths = [basedir + '/' +val for val in files]
dates = [fls[44:54].replace('.','/') for fls in filePaths]
...
for k in range(1140,finalPathsDF.shape[0]):
