...
def MakePanelCByBlock(DataFrame,column,block,cax):
    ...
    colors = [plt.cm.Blues(each) for each in dcolors]
...
colrs = [plt.cm.Blues(val) for val in np.linspace(0.5,1,num=len(Alphabet))]
...
data['ratio'] = data['totO3']/(data['spots']+1)
...
for k,lab in enumerate(labels):
    ...
    if lab=='date':
        ...
        cbar = fig.colorbar(cp,cax=axs1cb,cmap='Blues')
        cbar.ax.set_ylabel(names[k], rotation=270)
...
for k,lab in enumerate(labels):
    ...
    localRolling = data[Alphabet].groupby(pd.cut(data[lab],1000)).mean().rolling(q_rolling)
    rollingMean = localRolling.mean()
    rollingMean = (rollingMean-rollingMean.min())/(rollingMean.max()-rollingMean.min())
    ...
    cbar = fig.colorbar(cp,cax=axs7cb,cmap='Blues')
    cbar.ax.set_ylabel(names[k], rotation=270)

plt.tight_layout()
plt.savefig('/media/tavo/storage/images/image_dyn.png',dpi=75,bbox_inches='tight')
